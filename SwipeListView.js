...
import { bg_nodata } from '@look/assets/index';

/**
 * ListView that renders SwipeRows.
 */
class SwipeListView extends React.Component {
  constructor(props) {
    super(props);
    this._rows = {};
    ...
    if (!dataArray || dataArray.length === 0) {
      return (
        <View
          ...
          <Image
            source={bg_nodata}
            style={{ height: 160, width: 160, overflow: 'hidden' }}
          />
        </View>
      );

      //   return this.temporarilyNoData();
    }
    return isHiddenFoot ? null : (
      <BottomHint showFoot={showFoot} textValue={bottomLineText} />
    );
  }
